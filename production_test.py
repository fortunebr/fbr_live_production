...
def load_configuration():
    ...
    if exists:
        try:
            ...
            print(f'Required key "{e.args[0]}" not found.')
            logMessage(f'Required key "{e.args[0]}" not found.')
...
def webhook_discord(url, production, date, history) -> None:
...
def hourlyLogDisplay(data) -> str:
    ...
    for key, item in data:
        data_string += f"{key-1:02d}-{key:02d}: {item[0]}"
...
def main() -> None:
    ...
    try:
        ...
        prod_log[now_hour] = tuple(cur_production, cur_production_h)
    ...
    if cur_production_h > 0 and cur_production > 0 and prod_log:
        prod_detail = hourlyLogDisplay
        # Send to webhooks
        print(prod_detail)
        discord_url = wh.get("DISCORD", None)
        webhook_discord(
            url=discord_url, production=cur_production, date=now, history=prod_detail
        )
